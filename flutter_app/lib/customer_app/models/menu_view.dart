...
import 'package:flutter_app/customer_app/state/menu_service.dart';
...
class FoodViewModel extends ChangeNotifier {
  ...
  Future<void> getAllMenu() async {
    setLoading(true);
    var service = Service();
    var menu = await service.fetchAllMenu();
    if (menu != null) {
      menuLists = menu;
      setLoading(false);
    }
  ...
  Future<void> getdrinks() async {
    ...
    if (menuLists.isNotEmpty) {
      filteredDrinks =
          menuLists.where((element) => element.category == 'Drinks').toList();
    } else {
      // If foodLists is empty, fetch all menu and then filter drinks
      await getAllMenu();
      filteredDrinks =
          menuLists.where((element) => element.category == 'Drinks').toList();
    }
  ...
  Future<void> getfood() async {
    ...
    if (menuLists.isNotEmpty) {
      filteredFood =
          menuLists.where((element) => element.category == 'FoodItem').toList();
    } else {
      // If foodLists is empty, fetch all menu and then filter drinks
      await getAllMenu();
      filteredFood =
